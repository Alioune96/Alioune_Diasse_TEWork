<template>
  <div class="main">
    ...
    <div class="well-display">
      <div class="well" @click="setFilterLevel(0)">
        <span class="amount">{{ averageRating }}</span>
        Average Rating
      </div>

      <div class="well" @click="setFilterLevel(1)">
        <span class="amount">{{ numberOfOneStarReviews }}</span>
        1 Star Review{{ numberOfOneStarReviews === 1 ? '' : 's' }}
      </div>

      <div class="well" @click="setFilterLevel(2)">
        <span class="amount">{{ numberOfTwoStarReviews }}</span>
        2 Star Review{{ numberOfTwoStarReviews === 1 ? '' : 's' }}
      </div>

      <div class="well" @click="setFilterLevel(3)">
        <span class="amount">{{ numberOfThreeStarReviews }}</span>
        3 Star Review{{ numberOfThreeStarReviews === 1 ? '' : 's' }}
      </div>

      <div class="well" @click="setFilterLevel(4)">
        <span class="amount">{{ numberOfFourStarReviews }}</span>
        4 Star Review{{ numberOfFourStarReviews === 1 ? '' : 's' }}
      </div>

      <div class="well" @click="setFilterLevel(5)">
        <span class="amount">{{ numberOfFiveStarReviews }}</span>
        5 Star Review{{ numberOfFiveStarReviews === 1 ? '' : 's' }}
      </div>
    </div>

    <a href="#" v-if="!showForm" v-on:click.prevent="toggleShowForm">Add a new review!</a>
    ...
    <form v-else>
        ...
        <input type="submit" value="Save" v-on:click.prevent="saveForm" >
...
export default {
  ...
  methods: {
    ...
    saveForm() {
      if (this.newReview.reviewer && this.newReview.review && this.newReview.title && this.newReview.rating) {
  ...
  computed: {
    filteredReviews() {
      return this.reviews.filter((rvw) => {
        return this.filterLevel === 0 || rvw.rating === this.filterLevel;
      });
      ...
      //   return r.rating === this.filterLevel;
      // });
    },
    averageRating() {
