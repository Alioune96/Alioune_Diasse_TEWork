...
public class Exercise07_StoreHours {
    ...
    public boolean isStoreOpen(int currentHour, char currentDay) {
        if(isStoreOpen(currentHour) && currentDay == 'M'){
            return true;
        }
        else if (isStoreOpen(currentHour)  && currentDay == 'W'){
            return true;
        }
        else if(isStoreOpen(currentHour) && currentDay == 'F'){
            return true;
        }
        return false;
    }
    ...
    public boolean isStoreOpen(int currentHour, char currentDay, boolean isSummer) {
        boolean timeOf = isStoreOpen(currentHour, currentDay);

        if((currentDay == 'W' || currentDay == 'S') && isSummer == true){
            return true;
        }
        return timeOf;
    }
