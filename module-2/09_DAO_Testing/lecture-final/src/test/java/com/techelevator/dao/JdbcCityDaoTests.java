...
public class JdbcCityDaoTests extends BaseDaoTests {
    ...
    // Java objects to represent the mock data
    ...
    public void getCitiesByState_with_valid_state_returns_correct_cities() {
        List<City> cities = sut.getCitiesByState("AA"); // ACT
        ...
        cities = sut.getCitiesByState("BB"); // ACT
        Assert.assertNotNull(cities);
    ...
    public void createCity_creates_city() {
        City createdCity = sut.createCity(testCity); // Run the method, giving all info
        Assert.assertNotNull(createdCity);           // ASSERT output isn't null

        int newId = createdCity.getCityId();        // pull the ID out of the object (that we got back from the method)
        Assert.assertTrue(newId > 0);      // make sure it's a valid primary key!
