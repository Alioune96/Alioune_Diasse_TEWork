...
public class HotelController {
 ...
 /**
    ...
    public Reservation addReservation(@Valid @RequestBody Reservation reservation, Principal principal) {
        String username = (principal == null) ? "anon" : principal.getName();

        Reservation newReservation = reservationDao.createReservation(reservation);

        auditLog("Add", newReservation.getId(), username);
    ...
    @PreAuthorize("hasAnyRole('ADMIN', 'DORK')")
    @ResponseStatus(HttpStatus.NO_CONTENT)
    @RequestMapping(path = "/reservations/{id}", method = RequestMethod.DELETE)
    public void delete(@PathVariable int id) {
        if (reservationDao.deleteReservationById(id) == 0) {
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, "Reservation not found.");
        }
    }
